...
def is_route_feasible(route, node_types, demands, capa, depot_idx):
    ...
    # (vi) 최소 1개의 배송 노드 포함
    if not any(node_types[n] == 1 for n in route):
        return False

    # (v) 용량 제약 (요청한 방식)
    delivery_sum = sum(demands[n] for n in route if node_types[n] == 1)
    pickup_sum   = sum(demands[n] for n in route if node_types[n] == 0)
